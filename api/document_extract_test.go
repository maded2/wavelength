 ...
 *This document will be updated iteratively as we progress through the interview.*
 ...
 Some outro.`
	...
	t.Run("markdown horizontal rules in conversation do not trigger false extraction", func(t *testing.T) {
		response := `Here's a summary of what we discussed:
 ...
 Let me know if I missed anything.`
		...
		if doc != "" {
			t.Errorf("expected empty document (--- is not a valid delimiter), got: %q", doc)
		}
		if !strings.Contains(conv, "Key Points") {
			t.Errorf("expected full content as conversational, got: %s", conv)
		}
	})
}
